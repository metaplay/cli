...
func init() {
	...
	cmd := &cobra.Command{
		...
		Long: renderLong(o, `
			...
			re-apply the changes with 'patch -p1'. Some hunks may fail if there are conflicts
			with the new SDK version and will require manual resolution.
...
func (o *updateSdkOpts) Run(cmd *cobra.Command) error {
	...
	if len(modifications) > 0 {
		...
		log.Info().Msgf("After the update, you can re-apply the changes from the SDK directory with:")
