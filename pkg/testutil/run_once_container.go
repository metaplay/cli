...
func (r *RunOnceContainer) Run(ctx context.Context) (int, error) {
	...
	if r.opts.Network != "" {
		if req.HostConfigModifier == nil {
			req.HostConfigModifier = func(hc *dockercontainer.HostConfig) {
				hc.NetworkMode = dockercontainer.NetworkMode(r.opts.Network)
			}
		} else {
	...
	// Add port bindings if any (but skip if using container network mode)
	if len(portBindings) > 0 {
		// Check if we're using container network mode - if so, skip port bindings
		usingContainerNetwork := strings.HasPrefix(r.opts.Network, "container:")

		if !usingContainerNetwork {
			if req.HostConfigModifier == nil {
				req.HostConfigModifier = func(hc *dockercontainer.HostConfig) {
	...
	log.Debug().Msgf("Create run-once container: name=%s image=%s cmd=%v network=%s", r.opts.ContainerName, r.opts.Image, r.opts.Cmd, r.opts.Network)
